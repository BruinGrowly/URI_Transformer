...
def demonstrate():
    ...
    for i, phrase in enumerate(test_phrases):
        ...
        print(f"\n  Deep ICE Analysis:")
        print(f"    Intent:      {result.intent.purpose}, {result.intent.guiding_principles[0]}")
        print(f"    Context:     Primary Domain: '{result.context.primary_domain.value}', Validated: {result.context.is_valid}")
        print(f"    Execution:   {result.execution.description}")
